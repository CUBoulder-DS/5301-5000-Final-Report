...
```{r hyp-vs-CM}
#| label: tbl-hyp-vs-cm
#| tbl-cap: Proportionality Testing vs. Confusion Matrix Results
#| 
m<-c("FairFace","FairFace","DeepFace","DeepFace")
group<-c("60-69","Female, Given 'Other'","20-29, Given 'Black'","40-69")
p<-c("~1","2.58e-13","0.10","<=1.23e-5")
f<-c("0.354","0.922","0.588",">0.928")
#a<-c("0.671","0.914","","")
conc<-c(
	"High p-value != High F1 score",
	"Low p-value != Low F1 score",
	"High p-value != High F1 score",
	"Low p-value != Low F1 score"
)
tibble(
...
```{r tbl-conf-mat-sum}
#| label: tbl-conf-mat-sum
#| tbl-cap: Evaluation of Model Performance Using F1 Scores


m<-c("FairFace","DeepFace")
findings<-c(
"Preference in classifying the very young and old correctly.  Excellence in classifying gender given almost any other test categories.  Lack of excellence in racial classifications; preference in classification of Asian, White, and Black over Indian and Other.",
...
tibble(
	"Model"=m,
	"Findings"=findings
) %>% kable() %>% column_spec(1,width="2cm")%>%column_spec(2,width="13cm") #%>% knitr::kable()
```
...
```{r tbl-hyp-test-det}
#| label: tbl-hyp-test-detail
#| tbl-cap: Detailed Proportionality Testing Results
...
) %>% kable() %>% column_spec(c(1:2),width="2cm")%>%column_spec(c(3:4),width="6cm") #%>% knitr::kable()
```
...
```{r tbl-conf-mat-det}
#| label: tbl-conf-mat-detail
#| tbl-cap: Confusion Matrix - F1 Scoring Results
...
g_potential_bias <- c(
	"No groups meet 0.9 threshold. Preference in classifying the young and old correctly (0-9, 20-29, and 70-130); all other categories fall below 0.7, and between 0.1 to 0.4 below these top groups",
	"Both genders meet standard for excellence.",
	"None reach excellence. Preference for Asian, Black, White (Other, Indian fall 0.2-0.3 less than these groups)",
	...
	"None meet excellence. Bias against Indian, Other with F1 <= 0.4"
)
sub_g_potential_bias <-c(
	"Given gender, the top performing groups remain, with a preference for Female classification over Male in the groups. Given race, the very young meet excellence for 0-2, given Other, Asian, or White. 10-69 fall far behind given any racial group",
	"Given race, all genders meet excellence, except male, given Asian at 0.89. Given age, all genders meet excellence, less males given 0-9, and females given 0-2",
	"Given gender, model retains preference for classifying for Asian, White, Black. Given Age, excellence reached for White (given 3-9,60-69,)",
	...
	"No gender classifications meet excellence, given race. Preference for Male over Female for near all races (excluding Other). Preference for stronger male classification, given 40-69, and female, given 10-39.",
	"Poor for all races (given gender), with slightly lower performance for race classificaiton given a female subject."
)
tibble(
	"Model"=m,
	"Test Category"=cat,
	"Impacted Categories"=g_potential_bias,
	"Impacted Sub-Categories"=sub_g_potential_bias
)%>%kable() %>% column_spec(c(1:2),width="2cm")%>%column_spec(c(3:4),width="6cm") #%>% knitr::kable()
```
<!--PC-->
...
get_model_scores <- function(threshold) {
	...
	rowwise %>%
		mutate(
			p_value_classification=factor(test_result(model_p_value,model_power),levels=fac_lev),
...
```{r tbl-meta-score}
#| label: tbl-meta-scoring
#| tbl-cap: F1 and Accuracy Scores for Proportionality Tests as a Performance Measurer
...
extract_scores(cm_df,cm_ff,"DeepFace","FairFace",0.9) #%>% kable()
#kable(x)%>% kable_styling()
```
...
```{r tbl-pearson-corr}
#| label: tbl-pearson-correlation-pval-f1
#| tbl-cap: Pearson Correlation Between F1 Scores and Proportionality p-values
