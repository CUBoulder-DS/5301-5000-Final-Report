...
rename_f_d_cols = function(x) {
  rename_with(x, ~ paste(str_replace(., "fairface_", ""), "f", sep="_"), starts_with("fairface_")) %>%
    rename_with(~ paste(str_replace(., "deepface_", ""), "d", sep="_"), starts_with("deepface_"))
}
...
filter_pvalues_no_cond = function(category) {
  ...
  p_value_results %>%
    ...
    select(test_cat(category), fairface_p_value, deepface_p_value, 
           fairface_prop, deepface_prop, source_prop, fairface_n, deepface_n) %>%
...
* What is proportion? Clearly not proportion correct
...
#| tbl-cap: TODO 1 Table of F1 score and accuracy, by each category evaluated by the models

colname_fmt = "**<sub>%s:<br>%s</sub>**"
colnames = setNames(c(sprintf(colname_fmt, "Test Condition", "Race"),
                      sprintf(colname_fmt, "Test", "Category"),
                      sprintf(colname_fmt, "F1 Score", "DeepFace"),
                      sprintf(colname_fmt, "F1 Score", "FairFace"),
                      sprintf(colname_fmt, "Accuracy", "DeepFace"),
                      sprintf(colname_fmt, "Accuracy", "FairFace"),
                      sprintf(colname_fmt, "<sup>Proportion</sup>", "DeepFace"),
                      sprintf(colname_fmt, "<sup>Proportion</sup>", "FairFace"),
                      sprintf(colname_fmt, "p-value", "DeepFace"),
                      sprintf(colname_fmt, "p-value", "FairFace")), 
                    names(combined_results)[1:10])
...
combined_results %>%
  ...
  data_color(columns=9:10, method="bin", palette = c("#B1DC96", "#F2B8D2"), bins=c(0, 0.03, 1))

```

TODO 3 Table of F1 score, accuracy, p-value, and proportion correct by each category evaluated by the models
:::
:::

### p-value Critical Values

From the previous table, we extract and highlight key values; namely, the p-values where we fail to reject the null hypothesis at a significance level of 99.7%, displayed in @tbl-perf-selected.
...
tbl_gt = combined_results %>% 
  ...
  tab_stubhead("Model Test Condition, Category") %>%
...
# tbl_gt = gt_split(tbl_gt, row_every_n=5)
  ...
  # tab_stubhead("Model p-value") %>%
  # cols_label(.list= cells_body(rows = 2)) %>%
  # gt_split(col_slice_at = c( 8, 14))

tbl_gt
# gt_split(tbl_gt, row_every_n = 5)
```
