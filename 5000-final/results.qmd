...
get_f1_acc = function(df) {
  ...
  names = c(age[, "F1"] %>% names() %>% str_replace("Class", "Age"), 
            race[, "F1"] %>% names() %>% str_replace("Class", "Race"), 
            "Gender")
...
For each category and model, we calculate the F1 score and accuracy, as described in section 3. The results are summarized in @tbl-model-perf.
...
perf_table %>%
  # TODO: Try to color cells based on value, doesn't work right now
  # mutate_at(2:5, ~ cell_spec(., background = case_when(. > 0.8 ~ "green",
  #                                                   . <= 0.5 ~ "orangered",
  #                                                   .default = "white"))) %>%
  kbl(col.names = c("Category", "F1 Score", "Accuracy", "F1 Score", "Accuracy")) %>% 
  kable_styling(full_width = T) %>%
  column_spec(1, bold = T) %>%
  add_header_above(c(" " = 1, "FairFace" = 2, "DeepFace" = 2)) %>%
  pack_rows(index = c("Age" = 9, "Race" = 5, "Gender" = 1))
