...
t_tests <- function(df,age_col,t_prop,t_race,t_gender,t_age){
  ...
  #print(y)
  p_val <- t.test(x,y,var.equal=TRUE,conf.level=0.997)$p.value
  #do a power test
  #use +/- 5 as the effect size (so it's comparable to FairFace)
  #sig level of .997, two-sided test.
  test_power <- pwr.t2n.test(n1=length(x),n2=length(y),d=5,sig.level=.003,alternative = 'two.sided')$power
  res <- tibble(
    test_prop = t_prop,
    test_gender = t_gender,
    test_race = t_race,
...
DF_t_test_results <- samp_means_tests %>% 
  rowwise %>% 
  reframe(out=t_tests(deepface_df,'pred_age_DF_only',t_prop=test_prop,t_race=test_race,t_gender=test_gender,t_age=test_age)) %>% 
  ungroup() %>% unnest(cols=c(out))

#DF_t_test_results %>% write_csv("C:\\Users\\pconn\\OneDrive\\Desktop\\Stat-Git\\data\\DF_t_tests.csv")
