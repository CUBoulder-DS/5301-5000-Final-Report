...
```{r bootstrap_power_level}

get_sample_props <- function(vector,i,category){
  x <- vector[,i]
...
prop_bootstrap_data <- function(df,t_prop,t_race,t_gender,t_age){
  ...
  if (t_age!='All' & t_prop!='age_bins'){
    tdf <- df %>% filter(src_age_grp==t_age)
  }
  
  if (t_gender!='All' & t_prop!='genders'){
    tdf <- df %>% filter(src_gender==t_gender)
  }
  
  if (t_race!='All' & t_prop != 'races'){
    tdf <- df %>% filter(src_race == t_race)    
  }
  
  if (t_prop=='races'){
    src1 <- tdf %>% pull(src_race)
    pred1 <- tdf %>% pull(pred_race)
    catg <- t_race
  } else if (t_prop=='genders'){
    src1 <- tdf %>% pull(src_gender)
    pred1 <- tdf %>% pull(pred_gender)
    catg <- t_gender
  } else if (t_prop=='age_bins'){
    src1 <- tdf %>% pull(src_age_grp)
    pred1 <- tdf %>% pull(pred_age_grp)
    catg <- t_age
  } else {
    return(0)
  }
  
  #c(as.character(length(src1)),as.character(length(pred1)),catg,t_prop)
  # tibble(
  #   category = replicate(length(src1),catg),
  #   src = src1,
  #   pred = pred1
  # )
  
  source_bootstrap <- replicate(5000,sample(src1,size=500,replace=T))
  pred_bootstrap <- replicate(5000,sample(pred1,size=500,replace=T))

  source_props <- sapply(c(1:5000),get_sample_props,vector=source_bootstrap,category=catg)
  pred_props <- sapply(c(1:5000),get_sample_props,vector=pred_bootstrap,category=catg)

  tibble(
    model=replicate(5000,''),
    src_mean_props = source_props,
    pred_mean_props= pred_props
  )
}

proportion_bootstrap <- function(df,t_prop,t_race,t_gender,t_age){
  #add cases for prop all
  #get props
  sdf <- df
  pdf <- df
  if (t_age!='All' & t_prop!='age_bins'){
  ...
  #source_props
  p_val <- t.test(source_props,pred_props,var.equal=TRUE,conf.level=.997,alternative='two.sided')$p.value
  
  power <- pwr.t2n.test(n1=length(source_props),n2=length(pred_props),d=0.8,sig.level=.003,alternative='two.sided')$power
...
full_results %>% filter(model_p_value < .8) %>% rowwise %>% 
  reframe(out=proportion_bootstrap(
    deepface_df,
    test_prop,
    test_race,
    test_gender,
...
DF_samp_means %>% filter(test_race=='Indian',test_gender=='Male') %>% 
  ggplot()+
